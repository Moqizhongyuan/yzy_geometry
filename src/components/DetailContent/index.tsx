...
import GeometryMenu from '../../pages/detail/components/GeometryMenu'
import UploadLocalImg from '../../pages/detail/components/UploadLocalImg'
...
const DetailContent = () => {
  ...
  useEffect(() => {
    if (mainContainerRef.current) {
      mainContainerRef.current.style.height = 'calc(100vh - 40px)' // 减去NavBar的高度
    }
  }, [])
  ...
  return (
    ...
    >
            ...
            case 'circle':
              ...
              setLayers(prev => {
                ...
                res.unshift({
                  src: '',
                  active: true,
                  uuid: circleShape.uuid,
                  name: circleShape.name,
                  visible: circleShape.visible
                })
                return res
              })
              break
      ...
      />
      <div
        className={`flex-1 cursor-${cursor.current} ${style.canvas}`}
        style={{ height: '100%' }}
        ref={divRef}
      ></div>
      <div id="right" className="flex flex-col" style={{ height: '100%' }}>
        <div
          ref={divRightRef}
          id="effect"
